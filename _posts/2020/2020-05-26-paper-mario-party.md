...
1. Chance Cards
    * Indicated by a Mario-style [?] block, we made about a dozen of these. They have the following varieties:
    1. Get 1 coin (we made three of these)
    1. Lose 1 coin (we made three of these)
    1. Get 1 star (only one of these)
    1. Lose 1 star (only 1 of these)
    1. Move 1 space from the star (only 1 of these)
    1. The Star moves (only 1 of these)
1. Item Cards
    * Indicated by a 🍄, we made about a dozen of these as well. They have the following varieties:
    1. Dash Mushroom - adds 3 to your next dice roll (we made 3 of these)
	1. Poison Mushroom - subtracts 2 from another player's next dice roll (we made 3 of these)
	1. Golden Dash Mushroom - adds 5 to your next dice roll (we made 2 of these)
	1. Golden Pipe - move 1 place away from the star (only 1 of these)
	1. Double Card - allows the player to buy 2 stars the next time they land on the Star space (only 1 of these)
1. Minigame Cards
    * This is where the real fun is. At the end of each round, all players compete in a minigame for a coin. They should be fun games that kids can play which don't require a ton of equipment. Here are some of the ones we tried:
    1. Staring contest - Either 1 on 1 round-robin, or everyone at once. If you blink, you're out. Last person standing wins.
	1. Simon Says - One player is "Simon" and if they get everyone else out in a minute, they win, otherwise the winner(s) are whoever makes it to the end of the minute.
	1. Make a Basket - Pick some object to be the "ball" (we used a stuffed animal and an empty Amazon box), and take turns shooting "baskets," when you miss, you're out. Last person standing wins.
	1. Guess a Number - Everyone picks a number from 1-6 (you can't pick the same number as another player), and then one person rolls the dice. Who ever is closest without going over (a.k.a. the "The Price is Right" rule) wins.
	1. Rock, Paper, Scissors - Round-robin until one person is delcared the winner.
	1. Tic-tac-toe - Round-robin until one person is the winner, if the last game is a tie, both players get a coin.
	1. Footrace - Pick a starting and ending point and have a footrace. Winner gets a coin.
	1. Thumb wrestling - Round robin until one person is the winner.
1. Player Pieces
...
1. Each player starts with one Coin
1. Roll the dice to determine which Star Space to place a Star on
1. Have each player roll the dice to determine who goes first

### Taking Turns

Each turn goes like this:

1. If the player has an item card, they may choose to play it
1. Then, the player rolls the dice to determine how many spaces they will move
1. The player moves the number of spaces (you can move in any direction, but you can't "double back").
1. Depending on where the player lands:
    * Star Space
...
If anybody else gives this a shot, I'd love to hear how it worked and what tweaks or additions or house rules you made. The point is to have fun!